...
import es.daedalus.textalytics.gate.TextalyticsClass;
import es.daedalus.textalytics.gate.TextalyticsClass.category;
import es.daedalus.textalytics.gate.TextalyticsClass.category.term;
...
public static List<category> collectInfo(Element response) throws InvalidOffsetException, UnsupportedEncodingException {
    ...
    for(int i=0; i<nodeL.getLength(); i++) {
        ...
        es.daedalus.textalytics.gate.TextalyticsClass.category cat = new es.daedalus.textalytics.gate.TextalyticsClass.category();
        for(int j=0;j<features.getLength();j++){
            ...
            if ("code".equals(value_name)) {
                    ...
                    for(int k=0;k<terms.getLength();k++){
                        es.daedalus.textalytics.gate.TextalyticsClass.category.term t = new es.daedalus.textalytics.gate.TextalyticsClass.category.term();
