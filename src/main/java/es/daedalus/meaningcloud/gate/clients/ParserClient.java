...
public class ParserClient {
	...
	public static class Annot { // Implementation of an Annotation Object
								// without ID. It will be assigned to the
								// actual AnnotationSet in the core component.
								// It contains intra-sentence offsets
	...
	public static Recursive collectInfo(Element response_node, String nameNode,
			String dompos, String fatherID) throws InvalidOffsetException,
		...
		NodeList nodeL = (NodeList) expr1.evaluate(response_node,
		...
		for (int i = 0; i < nodeL.getLength(); i++) {
			...
			for (int j = 0; j < data_node.getLength(); j++) {
				...
				if (name.equals("form"))
					token.form = new String(n.getTextContent().getBytes(),
							"UTF-8");
				else if (name.equals("type"))
					token.type = new String(n.getTextContent().getBytes(),
							"UTF-8");
				else if (name.equals("normalized_form"))
					token.normalized_form = new String(n.getTextContent()
							.getBytes(), "UTF-8");
				else if (name.equals("bold"))
					token.bold = new String(n.getTextContent().getBytes(),
							"UTF-8");
				else if (name.equals("italic"))
					token.italic = new String(n.getTextContent().getBytes(),
							"UTF-8");
				else if (name.equals("underscore"))
					token.underscore = new String(
							n.getTextContent().getBytes(), "UTF-8");
				else if (name.equals("title"))
					token.title = new String(n.getTextContent().getBytes(),
							"UTF-8");
				else if (name.equals("separation"))
					token.separation = new String(
							n.getTextContent().getBytes(), "UTF-8");
				else if (name.equals("quote_level"))
					token.quote_level = new String(n.getTextContent()
							.getBytes(), "UTF-8");
				else if (name.equals("head"))
					token.head = new String(n.getTextContent().getBytes(),
							"UTF-8");
					...
					try {
						...
						for (int s_it = 0; s_it < syntTreeRel.getLength(); s_it++) {
							...
							for (int t_it = 0; t_it < dataList.getLength(); t_it++) {
								...
								if (dataName.equals("id")) {
									s.id = new String(data.getTextContent()
											.getBytes(), "UTF-8");
								} else if (dataName.equals("type")) {
									s.type = new String(data.getTextContent()
											.getBytes(), "UTF-8");
								}
							}
						...
						System.err.println("Exception in syntTreeRelations: "
								+ e.toString());
					}
					...
					try {
						...
						for (int a_it = 0; a_it < analyses.getLength(); a_it++) {
							...
							for (int b_it = 0; b_it < analysisList.getLength(); b_it++) {
								...
								if (dataName.equals("origin"))
									ana.origin = new String(data
											.getTextContent().getBytes(),
											"UTF-8");
								else if (dataName.equals("variety_dictionary"))
									ana.varietyDictionary = new String(data
											.getTextContent().getBytes(),
											"UTF-8");
								else if (dataName.equals("thematic_dictionary"))
									ana.thematicDictionary = new String(data
											.getTextContent().getBytes(),
											"UTF-8");
								else if (dataName.equals("tag"))
									ana.tag = new String(data.getTextContent()
											.getBytes(), "UTF-8");
								else if (dataName.equals("lemma"))
									ana.lemma = new String(data
											.getTextContent().getBytes(),
											"UTF-8");
								else if (dataName.equals("original_form"))
									ana.originalForm = new String(data
											.getTextContent().getBytes(),
											"UTF-8");
								else if (dataName.equals("tag_info"))
									ana.tagInfo = new String(data
											.getTextContent().getBytes(),
											"UTF-8");
								else if (dataName
										.equals("variety_dictionary_info"))
									ana.varietyDictionary = new String(data
											.getTextContent().getBytes(),
											"UTF-8");
								else if (dataName
										.equals("thematic_dictionary_info"))
									ana.thematicDictionary = new String(data
											.getTextContent().getBytes(),
											"UTF-8");
								else if (dataName.equals("remission"))
									ana.remission = new String(data
											.getTextContent().getBytes(),
											"UTF-8");
								else if (dataName.equals("check_info")) {
									NodeList checks = data.getChildNodes();
									for (int c_it = 0; c_it < checks
											.getLength(); c_it++) {
										Node check = checks.item(c_it);
										String checkName = check.getNodeName();
										if (checkName.equals("tag"))
											ana.checkInf.tag = new String(check
													.getTextContent()
													.getBytes(), "UTF-8");
										else if (checkName
												.equals("check_extra_info"))
											ana.checkInf.checkExtraInfo = new String(
													check.getTextContent()
															.getBytes(),
													"UTF-8");
										else if (checkName.equals("form_list")) {
											NodeList forms = check
													.getChildNodes();
											for (int d_it = 0; d_it < forms
													.getLength(); d_it++) {
												Node forms_form = forms
														.item(d_it);
												String formName = forms_form
														.getNodeName();
									...
									for (int c_it = 0; c_it < senseIds
											.getLength(); c_it++) {
										Node senseId = senseIds.item(c_it);
										String senseIdName = senseId
												.getNodeName();
										if (senseIdName.equals("sense_id"))
											ana.senseIds.add(new String(senseId
													.getTextContent()
													.getBytes(), "UTF-8"));
									}
								}
						...
						System.err.println("Exception in analyses: "
								+ e.toString());
					}
					...
					if (token.type.equals("sentence")) {
						...
						for (int subT_it = 0; subT_it < subTokenList
								.getLength(); subT_it++) {
					...
					Recursive h2 = collectInfo((Element) n, nameNode,
							"./token", token.id);
	...
	public static List<Annot> transform(List<TokenBean.Token_xml> tokenList,
			String nameNode) throws UnsupportedEncodingException {
			...
			try {
				...
				if (t.syntTreeRelationList.size() > 0) {
					...
					for (int k_it = 0; k_it < t.syntTreeRelationList.size(); k_it++) {
						if (k_it == 0) {
							syntTreeRelation_id_string += t.syntTreeRelationList
									.get(k_it).id;
							syntTreeRelation_type_string += t.syntTreeRelationList
									.get(k_it).type;
						} else {
							syntTreeRelation_id_string += ","
									+ t.syntTreeRelationList.get(k_it).id;
							syntTreeRelation_type_string += ","
									+ t.syntTreeRelationList.get(k_it).type;
						}
						fm.put("syntTreeRelation_" + (k_it + 1) + "_id",
								t.syntTreeRelationList.get(k_it).id);
						fm.put("syntTreeRelation_" + (k_it + 1) + "_type",
								t.syntTreeRelationList.get(k_it).type);
					}
					syntTreeRelation_id_string += "]";
					syntTreeRelation_type_string += "]";
					fm.put("syntTreeRelation_id_string",
							syntTreeRelation_id_string);
					fm.put("syntTreeRelation_type_string",
							syntTreeRelation_type_string);
				}
				if (t.analysisList.size() > 0) {
					...
					for (int k_it = 0; k_it < t.analysisList.size(); k_it++) {
						if (k_it == 0) {
							analysis_origin_string = ((!t.analysisList
									.get(k_it).origin.isEmpty()) ? t.analysisList
									.get(k_it).origin : "");
							analysis_variety_dictionary_string = ((!t.analysisList
									.get(k_it).varietyDictionary.isEmpty()) ? t.analysisList
									.get(k_it).varietyDictionary : "");
							analysis_thematic_dictionary_string = ((!t.analysisList
									.get(k_it).thematicDictionary.isEmpty()) ? t.analysisList
									.get(k_it).thematicDictionary : "");
							analysis_tag_string = ((!t.analysisList.get(k_it).tag
									.isEmpty()) ? t.analysisList.get(k_it).tag
									: "");
							analysis_lemma_string = ((!t.analysisList.get(k_it).lemma
									.isEmpty()) ? t.analysisList.get(k_it).lemma
									: "");
							analysis_original_form_string = ((!t.analysisList
									.get(k_it).originalForm.isEmpty()) ? t.analysisList
									.get(k_it).originalForm : "");
							analysis_tag_info_string = ((!t.analysisList
									.get(k_it).tagInfo.isEmpty()) ? t.analysisList
									.get(k_it).tagInfo : "");
							analysis_variety_dictionary_info_string = ((!t.analysisList
									.get(k_it).varietyDictionaryInfo.isEmpty()) ? t.analysisList
									.get(k_it).varietyDictionaryInfo : "");
							analysis_thematic_dictionary_info_string = ((!t.analysisList
									.get(k_it).thematicDictionaryInfo.isEmpty()) ? t.analysisList
									.get(k_it).thematicDictionaryInfo : "");
						} else {
							analysis_origin_string = ((!t.analysisList
									.get(k_it).origin.isEmpty()) ? ","
									+ t.analysisList.get(k_it).origin : ",");
							analysis_variety_dictionary_string = ((!t.analysisList
									.get(k_it).varietyDictionary.isEmpty()) ? ","
									+ t.analysisList.get(k_it).varietyDictionary
									: ",");
							analysis_thematic_dictionary_string = ((!t.analysisList
									.get(k_it).thematicDictionary.isEmpty()) ? ","
									+ t.analysisList.get(k_it).thematicDictionary
									: ",");
							analysis_tag_string = ((!t.analysisList.get(k_it).tag
									.isEmpty()) ? ","
									+ t.analysisList.get(k_it).tag : ",");
							analysis_lemma_string = ((!t.analysisList.get(k_it).lemma
									.isEmpty()) ? ","
									+ t.analysisList.get(k_it).lemma : ",");
							analysis_original_form_string = ((!t.analysisList
									.get(k_it).originalForm.isEmpty()) ? ","
									+ t.analysisList.get(k_it).originalForm
									: ",");
							analysis_tag_info_string = ((!t.analysisList
									.get(k_it).tagInfo.isEmpty()) ? ","
									+ t.analysisList.get(k_it).tagInfo : ",");
							analysis_variety_dictionary_info_string = ((!t.analysisList
									.get(k_it).varietyDictionaryInfo.isEmpty()) ? ","
									+ t.analysisList.get(k_it).varietyDictionaryInfo
									: ",");
							analysis_thematic_dictionary_info_string = ((!t.analysisList
									.get(k_it).thematicDictionaryInfo.isEmpty()) ? ","
									+ t.analysisList.get(k_it).thematicDictionaryInfo
									: ",");
						}

						if (!t.analysisList.get(k_it).origin.isEmpty())
							fm.put("analysis_" + (k_it + 1) + "_origin",
									t.analysisList.get(k_it).origin);
						if (!t.analysisList.get(k_it).varietyDictionary
								.isEmpty())
							fm.put("analysis_" + (k_it + 1)
									+ "_varietyDictionary",
									t.analysisList.get(k_it).varietyDictionary);
						if (!t.analysisList.get(k_it).thematicDictionary
								.isEmpty())
							fm.put("analysis_" + (k_it + 1)
									+ "_thematicDictionary",
									t.analysisList.get(k_it).thematicDictionary);
						if (!t.analysisList.get(k_it).tag.isEmpty())
							fm.put("analysis_" + (k_it + 1) + "_tag",
									t.analysisList.get(k_it).tag);
						if (!t.analysisList.get(k_it).lemma.isEmpty())
							fm.put("analysis_" + (k_it + 1) + "_lemma",
									t.analysisList.get(k_it).lemma);
						if (!t.analysisList.get(k_it).originalForm.isEmpty())
							fm.put("analysis_" + (k_it + 1) + "_originalForm",
									t.analysisList.get(k_it).originalForm);
						if (!t.analysisList.get(k_it).tagInfo.isEmpty())
							fm.put("analysis_" + (k_it + 1) + "_tagInfo",
									t.analysisList.get(k_it).tagInfo);
						if (!t.analysisList.get(k_it).varietyDictionaryInfo
								.isEmpty())
							fm.put("analysis_" + (k_it + 1)
									+ "_varietyDictionaryInfo",
									t.analysisList.get(k_it).varietyDictionaryInfo);
						if (!t.analysisList.get(k_it).thematicDictionaryInfo
								.isEmpty())
							fm.put("analysis_" + (k_it + 1)
									+ "_thematicDictionaryInfo",
									t.analysisList.get(k_it).thematicDictionaryInfo);
						if (!t.analysisList.get(k_it).remission.isEmpty())
							fm.put("analysis_" + (k_it + 1) + "_remission",
									t.analysisList.get(k_it).remission);
						if (!t.analysisList.get(k_it).checkInf.toString()
								.isEmpty()) {
							String analysis_checkInfo_tag_str = "[";
							String analysis_checkExtraInfo_str = "[";
							String analysis_checkInfo_form = "[";

							if (!t.analysisList.get(k_it).checkInf.tag
									.isEmpty())
								fm.put("analysis_" + (k_it + 1)
										+ "_checkInfo_tag",
										t.analysisList.get(k_it).checkInf.tag);
							if (!t.analysisList.get(k_it).checkInf.checkExtraInfo
									.isEmpty())
								fm.put("analysis_" + (k_it + 1)
										+ "_checkInfo_checkExtraInfo",
										t.analysisList.get(k_it).checkInf.checkExtraInfo);
							if (t.analysisList.get(k_it).checkInf.forms.size() > 0) {
								for (int l_it = 0; l_it < t.analysisList
										.get(k_it).checkInf.forms.size(); l_it++) {
									fm.put("analysis_" + (k_it + 1)
											+ "_checkInfo_form_" + (l_it + 1),
											t.analysisList.get(k_it).checkInf.forms
													.get(l_it));
								}
							}
						}
						if (t.analysisList.get(k_it).senseIds.size() > 0) {
							for (int l_it = 0; l_it < t.analysisList.get(k_it).senseIds
									.size(); l_it++) {
								fm.put("analysis_" + (k_it + 1) + "_senseId"
										+ (l_it + 1),
										t.analysisList.get(k_it).senseIds
												.get(l_it));
							}
						}
					}
				...
				if (t.senseList.size() > 0) {
					for (int k_it = 0; k_it < t.senseList.size(); k_it++) {
						fm.put("senses_" + (k_it + 1) + "_id",
								t.senseList.get(k_it).id);
						fm.put("senses_" + (k_it + 1) + "_info",
								t.senseList.get(k_it).info);
					}
				}
				a.fm = fm;
				a.Name = nameNode;
				...
				Logger.getLogger(ParserClient.class.getName()).log(
