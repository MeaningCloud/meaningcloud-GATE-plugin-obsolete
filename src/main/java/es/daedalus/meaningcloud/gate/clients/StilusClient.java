...
public class StilusClient {
	...
	public static class result {
		public String text = "", type = "", level = "", rule = "", msg = "",
				inip = "", endp = "inip";
	...
	public static List<result> collectInfo(Element response, String nameNode,
			gate.AnnotationSet outputAS/* , gate.AnnotationSet original */)
		...
		for (int i = 0; i < nodeL.getLength(); i++) {
			...
			for (int i_it = 0; i_it < res_li.getLength(); i_it++) {
				...
				if ("text".equals(res_name)) {
					...
					r.text = new String(result.getTextContent().getBytes(),
							"UTF-8");
				} else if ("type".equals(res_name)) {

					r.type = new String(result.getTextContent().getBytes(),
							"UTF-8");
				} else if ("rule".equals(res_name)) {

					r.rule = new String(result.getTextContent().getBytes(),
							"UTF-8");
				} else if ("msg".equals(res_name)) {

					r.msg = new String(result.getTextContent().getBytes(),
							"UTF-8");
					...
					for (int j = 0; j < sugs.getLength(); j++) {
						...
						for (int k = 0; k < values.getLength(); k++) {
							...
							if ("form".equals(name)) {
								s.form = new String(features.getTextContent()
										.getBytes(), "UTF-8");
							} else if ("confidence".equals(name)) {
								s.confidence = new String(features
